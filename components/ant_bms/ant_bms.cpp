...
bool AntBms::parse_ant_bms_byte_(uint8_t byte) {
  ...
  if (!(raw[0] == 0xAA && raw[1] == 0x55 && raw[2] == 0xAA && raw[3] == 0xFF) && !(raw[0] == 0x7E && raw[1] == 0xA1)) {
    ESP_LOGW(TAG, "Invalid header %x %x %x %x %d",raw[0],raw[1],raw[2],raw[3],at);
  ...
  {
      ESP_LOGW(TAG, "**************************************Valid header %x %x %x %x %d",raw[0],raw[1],raw[2],raw[3],at);
  }
...
void AntBms::send_(uint8_t function, uint8_t address, uint16_t value) {
  ...
  this->write_array(frame, 6);
    ESP_LOGW(TAG, "Send %x %x %x %x %x %x",frame[0],frame[1],frame[2],frame[3],frame[4],frame[5]);
  this->flush();
}
...
void AntBms::authenticate_v2021_() {
  ...
  this->write_array(frame, 22);
    ESP_LOGW(TAG, "Authentificate_v2021 %x %x %x %x %x %x",frame[0],frame[1],frame[2],frame[3],frame[4],frame[5]);
  this->flush();
}
...
void AntBms::authenticate_v2021_variable_(const uint8_t *data, uint8_t data_len) {
  ...
  this->write_array(frame.data(), frame.size());
    ESP_LOGW(TAG, "Authentificate_v2021_variable %x %x %x %x %x %x",frame[0],frame[1],frame[2],frame[3],frame[4],frame[5]);
  this->flush();
}
...
void AntBms::send_v2021_(uint8_t function, uint8_t address, uint16_t value) {
  ...
  this->write_array(frame, 10);
    ESP_LOGW(TAG, "Send_v2021 %x %x %x %x %x %x",frame[0],frame[1],frame[2],frame[3],frame[4],frame[5]);
  this->flush();
}
